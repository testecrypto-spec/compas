...
self.addEventListener("install", e => {
  // chaque fichier est mis en cache séparément : un fichier absent (pdf.js rangé ailleurs,
  // icône manquante) ne doit pas empêcher toute l'installation
  e.waitUntil(caches.open(VERSION).then(async c => {
    await c.addAll(["./", "index.html"]);
    await Promise.all(FICHIERS.filter(f => f !== "./" && f !== "index.html")
